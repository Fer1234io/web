<template>
    <div class="productos-mas-vendidos-por-mes">
      ...
      <!-- Formulario para generar la gráfica -->
    <div class="reporte-form">
      <h2>Generar Gráfica</h2>
      <label for="mes">Seleccionar Mes:</label>
      <select v-model="mesSeleccionado" id="mes">
  ...
  export default {
    ...
    methods: {
      ...
      try {
        const payload = this.mesSeleccionado
        ...
        const labels = [...new Set(this.chartData.map((item) => this.meses[item.mes - 1]))];
        const datasetsMap = {};
        ...
        this.chartData.forEach((item) => {
          ...
          const mesIndex = labels.indexOf(this.meses[item.mes - 1]);
